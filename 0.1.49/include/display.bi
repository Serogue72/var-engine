...
sub init_DISPLAY()
  ...
  for i as integer = 1 to 12
    DISPLAY.RESOLUTION_LIST( i, 0 ) = DISPLAY.SOURCE_W * i
    DISPLAY.RESOLUTION_LIST( i, 1 ) = DISPLAY.SOURCE_H * i
  next i
  
  screeninfo DISPLAY.DESKTOP_RESOLUTION( 0 ), DISPLAY.DESKTOP_RESOLUTION( 1 )
  
  for j as integer = 12 to 1 step -1
    if ( DISPLAY.RESOLUTION_LIST( j, 0 ) < DISPLAY.DESKTOP_RESOLUTION( 0 ) ) and _
      ( DISPLAY.RESOLUTION_LIST( j, 1 ) < DISPLAY.DESKTOP_RESOLUTION( 1 ) ) then
      DISPLAY.W = DISPLAY.RESOLUTION_LIST( j, 0 )
      DISPLAY.H = DISPLAY.RESOLUTION_LIST( j, 1 )
  ...
  screenres DISPLAY.W, _
            DISPLAY.H, _
            32,, _
            DISPLAY.SCREENRES_FLAG
  
  DISPLAY.SOURCE_PTR( 0 ) = imagecreate( DISPLAY.SOURCE_W, DISPLAY.SOURCE_H ) : if DISPLAY.SOURCE_PTR( 0 ) = 0 then end
  DISPLAY.SOURCE_PTR( 1 ) = imagecreate( DISPLAY.SOURCE_W, DISPLAY.SOURCE_H ) : if DISPLAY.SOURCE_PTR( 1 ) = 0 then end
  DISPLAY.SHADOW_LAYER_PTR = imagecreate( DISPLAY.SOURCE_W, DISPLAY.SOURCE_H ) : if DISPLAY.SHADOW_LAYER_PTR = 0 then end
  
  DISPLAY.ALPHA_VALUE( 0 ) = 255
  DISPLAY.ALPHA_VALUE( 1 ) = 255
end sub

sub change_resolution_DISPLAY()
  scale_step_DISPLAY()
  screenres DISPLAY.W, _
            DISPLAY.H, _
            32,, _
            DISPLAY.SCREENRES_FLAG
  change_resolution_CAMERA()
end sub
...
sub clear_buffer_DISPLAY()
  ...
  circle DISPLAY.SHADOW_LAYER_PTR, ( 159, 89 ), 145, rgb( 255, 0, 255 ), , , , F
end sub
